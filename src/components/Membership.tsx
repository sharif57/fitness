...
//   // Check if user is authenticated (example: using localStorage token)

//   const isAuthenticated = !!localStorage.getItem("accessToken"); // Adjust based on your auth system

//   // Fetch all packages and the user's subscription info
//   const { data, error, isLoading } = useAllPackageGetQuery(undefined);
//   const { data: userSubscription } = useSubscriptionGetQuery(undefined, {
//     skip: !isAuthenticated, // Skip fetching subscription if not logged in
//   });
//   const [subscription] = useSubscriptionMutation();
//   const [subscriptionUpdate] = useSubscriptionUpdateMutation();
...
//   const handleSubscription = async (packageId: string) => {
//     if (!isAuthenticated) {
//       toast.info("Please log in to purchase a subscription.");
//       router.push("/login");
//       return;
//     }
...
//   const handleUpdateSubscription = async (newPackageId: string) => {
//     if (!isAuthenticated) {
//       toast.info("Please log in to update your subscription.");
//       router.push("/login");
//       return;
//     }
...
const Membership: React.FC = () => {
  ...
  useEffect(() => {
    setIsClient(true); // Confirm we're on the client side
  ...
  const { data, error, isLoading } = useAllPackageGetQuery(undefined);
  const { data: userSubscription } = useSubscriptionGetQuery(undefined, {
    skip: !isAuthenticated || !isClient, // Skip fetching subscription if not logged in or not on client
  });
  const [subscription] = useSubscriptionMutation();
  const [subscriptionUpdate] = useSubscriptionUpdateMutation();
